...
Router.post('/createorder', async (req, res) => {
    ...
    try{
        ...
        if (!findcustomer){
            res.status(404).json({message: "Não foi localizado nenhum cliente com este ID, favor verifique e tente novamente.", error: true, data: req.body})
            return
        }
        ...
        res.status(200).json({message: 'Criação de pedido feito com sucesso!', error: false, data: createorder.rows})
    }catch(err){
        console.log(err)
...
Router.put('/updateorder', async (req, res) => {
    ...
    try{
        ...
        if (vl_id.rowCount == 0){
            res.status(404).json({message: "Não foi encontrado nenhum pedido com este ID!", error: true})
        return
        }
        ...
        res.status(200).json({message: `${textatts} atualizado(s) com sucesso!`, error: false})
